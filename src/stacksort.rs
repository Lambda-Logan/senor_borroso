...
/*
struct UnwrapDual<L, T, R>(DualIter<L, T, R>);

impl<L, T, R> Iterator for UnwrapDual<L, T, R>
where
    T: Ord,
    L: Iterator<Item = T>,
    R: Iterator<Item = T>,
...
enum MergedStackIter<'a, I, T> {
    Head(DualIter<I, T, &'a mut MergedStackIter<'a, I, T>>),
    Nil,
}

impl<'a, I, T> Iterator for MergedStackIter<'a, I, T>
where
    T: Ord,
    I: Iterator<Item = T>,
{
